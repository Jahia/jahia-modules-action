...
export async function prepareTestrailMetadata(
  ...
  if (
    fs.existsSync(platformDataFile) &&
...
export async function publishToTestrail(
  ...
  if (fs.existsSync(reportsPath)) {
    ...
    command += ` --profile="${options.profile}"` // Adding profile name to include it in testrail-run name
    ...
    core.info(
      `ERROR: The following path does not exist: ${reportsPath}, report will not be submitted to testrail`
    )
  }
}
